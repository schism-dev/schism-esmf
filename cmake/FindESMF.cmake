...
if(EXISTS ${ESMFMKFILE})
  ...
  set(ESMF_INCLUDE_DIRS ${ESMF_INCLUDE_DIRECTORIES} CACHE INTERNAL "ESMF Include Directories")
  set(ESMF_LIBRARIES ${ESMF_LIBRARY_LOCATION} CACHE INTERNAL "ESMF Libraries") # Or rely on ESMF::ESMF target

  set(ESMF_COMPILE_DEFINITIONS "")
  if(ESMF_F90COMPILECPPFLAGS)
      string(REGEX MATCHALL "-D[^ ]+" _esmf_defs "${ESMF_F90COMPILECPPFLAGS}")
      foreach(_def ${_esmf_defs})
          string(REPLACE "-D" "" _def_val "${_def}")
          list(APPEND ESMF_COMPILE_DEFINITIONS ${_def_val})
      endforeach()
  endif()
  ...
  find_package_handle_standard_args(
        ...
        REQUIRED_VARS ESMF_LIBRARY_LOCATION ESMF_INCLUDE_DIRS ESMF_LIBRARIES # ESMF_INTERFACE_LINK_LIBRARIES ESMF_F90COMPILEPATHS
        VERSION_VAR ESMF_VERSION)

  set_target_properties(ESMF::ESMF PROPERTIES
        IMPORTED_LOCATION "${ESMF_LIBRARY_LOCATION}"
        INTERFACE_INCLUDE_DIRECTORIES "${ESMF_INCLUDE_DIRECTORIES}" # Should be ESMF_INCLUDE_DIRS
        INTERFACE_LINK_LIBRARIES "${ESMF_INTERFACE_LINK_LIBRARIES}") # This might need to be more structured if ESMF_LIBRARIES is just the .so/.a
