...
subroutine Initialize(comp, importState, exportState, clock, rc)
  ...
  do i=1,npa
    ...
    if (lreadll) then
      ...
      nodecoords2d(2*i-1) = rad2deg*xlon(i)
      nodecoords2d(2*i)   = rad2deg*ylat(i)
    else
      ! use cartesian coordinates
      nodecoords2d(2*i-1) = xnd(i)
      nodecoords2d(2*i)   = ynd(i)
    end if
    if (i<=np) then
      nodeowners(i)       = ipgl(iplg(i))%rank
    else
  ...
  do i=1,nea
    ...
    elementcoords2d(2*i-1)=sum(nodecoords2d(2*elnode(1:i34(i),i)-1))/i34(i)
    elementcoords2d(2*i)=sum(nodecoords2d(2*elnode(1:i34(i),i)))/i34(i)
  end do
...
#define ESMF_METHOD "Finalize"
  ...
  use schism_glbl, only: tr_el, kbe, wsett, rkind, nea, nvrt
  implicit none
  ...
  use schism_glbl, only: nea, kbe, tr_el, rkind, ze, idry_e
  implicit none
