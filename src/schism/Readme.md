...
The ESMF cap `schism_esmf_cap` relies on utilities in `schism_esmf_util` and the basic model interface `schism_bmi`.
The NUOPC cap `schism_nuopc_cap` relies on utilities in `schism_nuopc_util`and `schism_esmf_util` and the `schism_bmi`.
