...
MODULE obs_A_pdafomi
  ...
  INTEGER :: itype=1        ! Specify different type obs from init_dim_obs_all.F90
...
! *********************************************
    ...
    IF (mype_filter==0) &
         WRITE (*,'(8x,a)') 'Assimilate observations - OBS_A'
    ...
    if (Vlocal_opt.eq.1) then
       thisobs%disttype = 10   ! 10=Cartesian ! Consider to use 1 for global
                               ! For 2D+1D setup
...
! ***********************************************************
    ...
    if (dim_obs_p.gt.0) then
       ...
       !Debug
       !IF (mype_filter==0) write(*,'(a,3f8.2)') 'In obs_A_omi: x,y,z=',ocoord_p2(:,10)
