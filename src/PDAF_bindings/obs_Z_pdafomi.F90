!$Id: obs_Z_pdafomi.F90 912 2021-12-01 18:04:51Z lnerger $
...
MODULE obs_Z_pdafomi
  ...
  LOGICAL :: assim_Z        !< Whether to assimilate this data type
  INTEGER :: itype=2        ! Specify different type obs from init_dim_obs_all.F90
...
!!
  SUBROUTINE init_dim_obs_Z(step, dim_obs)
    ...
    if (dim_obs_p.gt.0) then
       ...
       do i=1,dim_obs_p
          ...
          if (ics==2) then
             ...
             if (Vlocal_opt.eq.1) ocoord_p2(3,i) = obs_coords_p(5,idx)
          else
...
!!
  SUBROUTINE obs_op_Z(dim_p, dim_obs, state_p, ostate)
...
!!
  SUBROUTINE init_dim_obs_l_Z(domain_p, step, dim_obs, dim_obs_l)
...
! **********************************************
  ...
  END SUBROUTINE init_dim_obs_l_Z
...
!!
  SUBROUTINE localize_covar_Z(dim_p, dim_obs, HP_p, HPH, coords_p)
...
! *************************************
  ...
  END SUBROUTINE localize_covar_Z
...
! ***********************************
  ...
  END SUBROUTINE deallocate_obs_Z

END MODULE obs_Z_pdafomi
